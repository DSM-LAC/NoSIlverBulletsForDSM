...
#name <- c('ARGENTINA', 'BELIZE', 'BOLIVIA','BRAZIL','CHILE','COLOMBIA',
 ...
 #lis <- lis[grep('RESIDUALmapQRF-LACensembleSOC.tif', lis)]
...
#x <- mask (x , cou)
#writeRaster(x, file=paste0(name[i], 'maskedV2RESIDUALmapQRF-#LACensembleSOC.tif'), 
#overwrite=TRUE)
#print(name[i])}
...
wgs84.tif.list <- list.files(path=getwd(), pattern=glob2rx("*RESIDUALmapQRF-LACensembleSOC.tif"), full.names=T,recursive=F)
...
wgs84.tif.list <- list.files(path=getwd(), pattern=glob2rx("*maskedV2RESIDUALmapQRF-LACensembleSOC.tif"), full.names=T,recursive=F)
...
writeRaster(GSIF::merge(mos2, mos), file='SOC-ensembleRESIDUALmap-country-specific.tif')
